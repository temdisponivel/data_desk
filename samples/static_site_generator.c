...
int main(int argument_count, char **arguments)
{
 ...
 SiteInfo site_info = {0};
 {
  ...
  DD_Node *site_info_file = DD_ParseWholeFile(site_info_path);
  site_info = ParseSiteInfo(site_info_file);
 }
 
 //~ NOTE(rjf): Parse pages.
 DD_Node *first_root = DD_NilNode();
 DD_Node *last_root = DD_NilNode();
 {
 ...
 {
  for(DD_EachNode(root, first_root))
  {
   for(DD_EachNode(node, root->first_child))
   {
    if(!DD_NodeIsNil(node->first_child) && DD_StringMatch(node->string, DD_S8Lit("index"), DD_StringMatchFlag_CaseInsensitive))
    {
     for(DD_EachNode(index_string, node->first_child))
     {
 ...
 for(DD_EachNode(root, first_root))
 {
  ...
  if(file)
  {
   ...
   DD_String8 site_title      = !DD_NodeIsNil(site_info.title) ? site_info.title->string : DD_S8Lit("");
   DD_String8 title           = !DD_NodeIsNil(page_info.title) ? page_info.title->string : DD_S8Lit("");
   DD_String8 url             = !DD_NodeIsNil(site_info.canonical_url) ? site_info.canonical_url->string : DD_S8Lit("");
   DD_String8 author          = !DD_NodeIsNil(site_info.author) ? site_info.author->string : DD_S8Lit("");
   DD_String8 twitter_handle  = !DD_NodeIsNil(site_info.twitter_handle) ? site_info.twitter_handle->string : DD_S8Lit("");
   ...
   // NOTE(rjf): Generate body.
   {
    ...
    if(page_info.parent)
    {
     fprintf(file, "<div class=\"standalone_link_container\"><a class=\"link\" href=\"%.*s.html\">Ã¢â€ Â Back</a></div>", DD_StringExpand(page_info.parent->string));
    }
    ...
    if(page_info.header_image)
    {
     fprintf(file, "<div class=\"page_banner\" style=\"background-image: url('%.*s');\"></div>",
             DD_StringExpand(page_info.header_image->string));
    }
...
{
 PageInfo info = {0};
 for(DD_EachNode(node, page->first_child))
 {
  if(node->kind == DD_NodeKind_Set && !DD_NodeIsNil(node->first_child))
  {
   ...
   else if(DD_StringMatch(node->string, DD_S8Lit("header_image"), DD_StringMatchFlag_CaseInsensitive))
   {
...
{
 SiteInfo info = {0};
 for(DD_EachNode(node, site->first_child))
 {
  if(node->kind == DD_NodeKind_Set && !DD_NodeIsNil(node->first_child))
  {
   if(DD_StringMatch(node->string, DD_S8Lit("title"), DD_StringMatchFlag_CaseInsensitive))
...
MakeDateString(DD_Node *date)
{
 ...
 if(date)
 {
  ...
  for(DD_EachNode(child, date->first_child))
  {
 ...
 switch(node->kind)
 {
  ...
  case DD_NodeKind_Set:
  {
   if(DD_NodeHasTag(node, DD_S8Lit("list")))
   {
    ...
    {
     if(DD_NodeIsNil(child->first_child))
     {
